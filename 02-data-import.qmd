...
Arabesque also allows you to import your own datasets via the home page, in order to create a new map.

You can import your datastes using a web link (possible with datasets from CorText) or load .csv files.

At least one .CSV file depicting links/flow origin-destination data file is required, formatting with three columns. A node file is also required for the geolocalisation of the origin and destination places for custom localisation file.

![](images/Arabesque_homepage_newmap.png)

For this example, we will use for example the historical trade flows listed in the mobscol file. For more information about this data set see in arabesque-examples.
...
> Example: the Mobscol origin-destination matrice.
>
> .CSV format
>
...
Do not modify this .zip file, otherwise it will no longer work with Arabesque, and you will not be able to load your work space.
:::
